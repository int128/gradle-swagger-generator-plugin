...
class OpenAPI3AdaptorSpec extends Specification {
    @Canonical
    ...
    def "generate should build options with #c.generateOptions"() {
        ...
        when:
        def actual = new OpenAPI3Adaptor().generate(c.generateOptions)
        ...
        c << [
            new TestCase(
                ...
                javaExecOptions: new JavaExecOptions(
                    ...
                    main: OpenAPI3Adaptor.CLASS_NAME,
                    args: [
                        'generate',
                        '-g', 'java',
                        '-i', 'input',
    ...
    def 'help should build options'() {
        ...
        when:
        def actual = new OpenAPI3Adaptor().help(options)
    ...
    def 'configHelp should build options'() {
        ...
        def expected = new JavaExecOptions(
            classpath: [new File('foo.jar')],
            main: OpenAPI3Adaptor.CLASS_NAME,
            args: ['config-help', '-g', 'java'],
            systemProperties: SLF4J_PROPERTIES,
        )

        when:
        def actual = new OpenAPI3Adaptor().configHelp(options)
